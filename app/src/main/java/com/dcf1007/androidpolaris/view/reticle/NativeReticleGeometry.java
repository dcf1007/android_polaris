 ...
 *
 * <p>The object tree keeps the original high-level SVG group ids so the overlay can rotate the
 * same logical parts that the browser version manipulated: polar scope, date scale, fixed time
 * scale, active HA pointer, fixed 0h pointer and Polaris marker. All coordinates remain in the
 * original SVG viewBox, so this is a native Canvas overlay object rather than a WebView/SVG layer.</p>
 */
