 ...
 *
 * <p>The app does not ship or parse a vector file at runtime. The source drawing has already been
 * translated into Java objects in {@link NativeReticleGeometry}. This class is only the drawing
 * engine used by those objects: groups preserve the original hierarchy, and leaf nodes draw native
 * Android Canvas primitives.</p>
 */
public final class NativeCanvasReticle {
    /** Text id used by the v4 source drawing for the active hour-angle value. */
    public static final String ID_HOUR_ANGLE_VALUE_TEXT = "ha_x2F_value";

    /** Text id used by the v4 source drawing for the fixed 0h date value. */
    public static final String ID_ZERO_HOUR_DATE_TEXT = "date_x2F_value";

    private NativeCanvasReticle() {}

    /** Common interface implemented by every drawable reticle node. */
    public interface ReticleNode {
        void draw(Canvas canvas, RenderContext context);
    }

    /**
     * Draw group preserving source hierarchy and group-level rotation.
     *
     * <p>The astronomy code supplies the live rotation values. The fixed time scale remains static,
     * the date/polar-scope layers rotate together, and the active HA pointer rotates independently.</p>
     */
    public static final class Group implements ReticleNode {
        ...
        public Group line(String elementId, float x1, float y1, float x2, float y2,
                          int strokeColor, float strokeWidth, float[] dashPattern) {
            add(new Line(elementId, x1, y1, x2, y2, strokeColor, strokeWidth, dashPattern));
            return this;
        }

        public Group circle(String elementId, float cx, float cy, float radius,
                            int fillColor, int strokeColor, float strokeWidth) {
            add(new Circle(elementId, cx, cy, radius, fillColor, strokeColor, strokeWidth));
            return this;
        }

        public Group path(String elementId, String pathData, int fillColor,
                          int strokeColor, float strokeWidth, float[] dashPattern) {
            add(new VectorPath(elementId, pathData, fillColor, strokeColor, strokeWidth, dashPattern));
            return this;
        }

        public Group text(String elementId, float x, float y, String text,
                          int fillColor, float textSize, boolean bold) {
    ...
    private static final class Line extends Element {
        ...
        Line(String id, float x1, float y1, float x2, float y2,
             int strokeColor, float strokeWidth, float[] dashPattern) {
            super(id);
            this.x1 = x1;
            this.y1 = y1;
        ...
        public void draw(Canvas canvas, RenderContext context) {
            ...
            // Keep the Polaris ray object in the tree, but drive its endpoint from live astronomy.
    ...
    private static final class VectorPath extends Element {
        ...
        VectorPath(String id, String pathData, int fillColor, int strokeColor,
                   float strokeWidth, float[] dashPattern) {
            super(id);
            this.path = createPathFromPathData(pathData);
            this.fillColor = fillColor;
            this.strokeColor = strokeColor;
            this.strokeWidth = strokeWidth;
            this.dashPattern = dashPattern;
    ...
    private static final class Text extends Element {
        ...
        private String getRenderedText(RenderContext context) {
            ...
            if (ID_HOUR_ANGLE_VALUE_TEXT.equals(id)) return UiFormatting.formatHours(context.result.activeHourAngleHours);
    ...
    /** Shared drawing state for one overlay pass. */
    public static final class RenderContext {
        ...
        float getRotationForGroup(String groupId) {
            ...
            // The 0h indicator and time scale remain fixed.
            return 0.0f;
        }
    }

    /** Parses the small path-command subset used by the reticle arrows. */
    private static Path createPathFromPathData(String pathData) {
        ...
        while (index < tokens.size()) {
            ...
            if (isPathCommandToken(token)) {
                ...
                throw new IllegalArgumentException("Path data starts without a command: " + pathData);
            }
            ...
            switch (command) {
                ...
                default:
                    throw new IllegalArgumentException("Unsupported path command '" + command + "' in: " + pathData);
            }
        }
        return path;
    }

    private static List<String> tokenizePathData(String pathData) {
